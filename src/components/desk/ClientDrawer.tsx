...
export function ClientDrawer({
  ...
  const [services, setServices] = useState<ClientServices>(normalizeServices(client?.services));
  ...
  useEffect(() => {
    ...
    setMaDueDay(client.maDueDay || 15);
    setServices(normalizeServices(client.services));
    setSoftware(client.software || 'Xero');
    setJunior(client.junior || '');
    setSenior(client.senior || '');
    setManager(client.manager || 'JD');
  }, [client?.id]);
  ...
  const sittingRows = clientObligations.filter((o) => o.workstream !== "metka_bas");
  const openObligations = sittingRows.filter(
    (o) => o.status !== "Done" && o.status !== "Not applicable",
  );
  const completedObligations = sittingRows.filter((o) => o.status === "Done");

  const persistClient = async (overrides: Partial<Client> = {}) => {
    await onUpdateClient(client.id, {
      notes,
      maDueDay: Number(maDueDay) || 0,
      services,
      software,
      junior,
      senior,
  ...
  const handleToggleService = (key: keyof ClientServices) => {
    const next = normalizeServices({ ...services, [key]: !services[key] });
    setServices(next);
    setServiceError(null);
    void persistClient({ services: next }).catch((err: unknown) => {
      setServices(services);
      setServiceError(err instanceof Error ? err.message : "Could not save services");
    });
  };
  ...
  const handleSaveClientDetails = async () => {
    ...
    try {
      ...
      setServiceError(err instanceof Error ? err.message : "Could not save client");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleClose = () => {
    void persistClient().finally(() => onClose());
  };
          ...
          <div>
            ...
            {openObligations.length === 0 ? (
              <div className="rounded-lg border border-dashed border-slate-200 p-6 text-center text-slate-500 text-xs">
                {isMetkaClient(client.id)
                  ? "No sitting rows. Entity pack cells live on Trackers, not this list."
